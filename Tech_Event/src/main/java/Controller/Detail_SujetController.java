...
public class Detail_SujetController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            if(AffichageForumController.getSujet_à_ouvrir().getId_User()==StartFXMain.user.getId_User())
            {
                id_jaime.setVisible(false);
                 id_jaimepas.setVisible(false);
            } else {
            if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(StartFXMain.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet()) ==0)
            {
                 id_jaime.setVisible(true);
                 id_jaimepas.setVisible(false);
            }
            else if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(StartFXMain.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet()) ==1) {
                id_jaime.setVisible(true);
                 id_jaimepas.setVisible(false);
            }else {
                id_jaime.setVisible(false);
    ...
    private void afficher_liste_commentaires() throws SQLException {
        ...
        ff.forEach(e -> {
            ...
            if (e.getId_User() == StartFXMain.user.getId_User()) {
...
id_send.setVisible(false);
 ...
 sujetService.compterjaime(AffichageForumController.getSujet_à_ouvrir());
        ...
        if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(StartFXMain.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet()) == 0
                ...
                jaimeService.ajouterJaime(new Jaime(StartFXMain.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet(), 2));
   
        }
        else if(jaimeService.FindValeurJaimeByIdUserAndIdSujet(StartFXMain.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet()) ==1) {
               jaimeService.incrementerjaime(StartFXMain.user.getId_User(),  AffichageForumController.getSujet_à_ouvrir().getId_Sujet());
                }
        id_nbre_jaime.setText(String.valueOf(sujetService.FindNombreJaimeById_Sujet(AffichageForumController.getSujet_à_ouvrir().getId_Sujet())));
  ...
  jaimeService.decrementerjaime(StartFXMain.user.getId_User(),  AffichageForumController.getSujet_à_ouvrir().getId_Sujet());
